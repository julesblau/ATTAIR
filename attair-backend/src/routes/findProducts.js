...
router.post("/", requireAuth, async (req, res) => {
  ...
  try {
    ...
    const [profileResult, scanResult, prefProfile, styleDnaResult] = await Promise.all([
      supabase.from("profiles").select("budget_min, budget_max, size_prefs, preference_profile, hanger_taste_cache").eq("id", req.userId).single(),
    ...
    const hangerTaste = profile?.hanger_taste_cache || null;
