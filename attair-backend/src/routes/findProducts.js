...
router.post("/", requireAuth, async (req, res) => {
  ...
  try {
    ...
    const [profileResult, scanResult, prefProfile, styleDnaResult] = await Promise.all([
      supabase.from("profiles").select("budget_min, budget_max, size_prefs, preference_profile, hanger_taste_cache, style_interests").eq("id", req.userId).single(),
    ...
    // Use hanger taste if available, otherwise build cold-start from onboarding style_interests
    let hangerTaste = profile?.hanger_taste_cache || null;
    if (!hangerTaste && Array.isArray(profile?.style_interests) && profile.style_interests.length > 0) {
      // Convert onboarding picks into a style_breakdown for cold-start personalization
